...
PACKAGE = "tai42_monitoring_langfuse"
...
# name (e.g. ``_cffi_backend`` from ``cffi``, ``_openssl`` from ``cryptography``).
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai42_monitoring_langfuse graph failed:\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai42_monitoring_langfuse module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai42_monitoring_langfuse sources: {offenders}"
