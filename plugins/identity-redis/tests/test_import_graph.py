...
``tai42_identity_redis`` is on the allowlist (its declared dependency closure plus
...
# plus their resolved closure. Compiled extensions appear under their registered
# top-level name (``_cffi_backend`` from ``cffi``; ``_openssl`` from
# ``cryptography``'s Rust binding).
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai42_identity_redis graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai42_identity_redis module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai42_identity_redis sources: {offenders}"
