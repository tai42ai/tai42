"""Bind a null app to the ``tai42_app`` handle before any package import, and provide the
curl-backed app and SSRF-guard fixtures shared across the tool suites.

The WhatsApp tools register through ``tai42_app`` at import time, so a no-op app must be bound
before importing a module; individual tests rebind to a capturing or curl-backed fake. The null app
records each tool's declared tags so the native-tags test can read them back.
"""
...
def curl_app() -> Iterator[None]:
    """Bind an app whose ``clients.client_ctx`` is tai42-kit's real pool, so the tools run against a
    live curl session (a loopback stub). Restores the prior app."""
...
def local_server() -> Iterator[_LocalServer]:
    """A real localhost HTTP server on an ephemeral port for the redirect-target test (the curl
    client cannot be intercepted by a transport-level mock): a redirect target that records every
    request it receives, so a test can prove it was never contacted."""
...
def _guard_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    disable the guard by default (the Graph calls hit a loopback stub it would block)."""
    import os
