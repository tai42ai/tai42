"""Fixtures for the WhatsApp tool tests.
...
The ``stub_server`` fixture stands in for the Meta Graph API and ``whatsapp_env`` configures the
tools' settings through real env parsing, so no test reaches a real Graph host.
"""
...
def load_registrations() -> Iterator[Callable[[str], _CaptureApp]]:
    """Return a loader that (re)imports a module under a capturing app and hands back the app so the
    test can read ``app.tools.registered``. The previously bound app is restored afterwards."""
...
class _Stub:
    ...
    current ``responder`` (default an empty-object 200), so a test can route by path or count."""
...
def stub_server() -> Iterator[_Stub]:
    """A real localhost server recording every request and answering via a settable responder, used
    as the Meta Graph API."""
...
@pytest.fixture
def whatsapp_env(monkeypatch: pytest.MonkeyPatch) -> Iterator[Callable[..., None]]:
    """Configure the WhatsApp settings through real env parsing (so empty-string cases exercise the
    production path) and clear the settings cache. Every ``CHANNEL_WHATSAPP_*`` name is removed
    first so the ambient environment cannot leak in."""
    for key in list(os.environ):
        if key.startswith("CHANNEL_WHATSAPP_"):
            monkeypatch.delenv(key, raising=False)

    def configure(
        *,
        access_token: str | None = "tok-123",
        waba_id: str | None = "WABA_1",
        api_base_url: str | None = None,
        timeout: float | None = None,
    ) -> None:
        if access_token is not None:
            monkeypatch.setenv("CHANNEL_WHATSAPP_ACCESS_TOKEN", access_token)
        if waba_id is not None:
            monkeypatch.setenv("CHANNEL_WHATSAPP_WABA_ID", waba_id)
        if api_base_url is not None:
            monkeypatch.setenv("CHANNEL_WHATSAPP_API_BASE_URL", api_base_url)
        if timeout is not None:
            monkeypatch.setenv("CHANNEL_WHATSAPP_HTTP_TIMEOUT_SECONDS", str(timeout))
        reset_all_settings()
