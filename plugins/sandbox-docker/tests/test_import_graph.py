...
``tai42_sandbox_docker`` is on the allowlist (the Docker Engine API client, the shared
platform substrate, their dependency closure, and the standard library only). The
runtime walk imports the package and every submodule in a fresh subprocess and inspects
``sys.modules``; the static walk parses every source file and collects import roots at
any nesting depth, catching imports nested in functions or ``TYPE_CHECKING`` blocks that
never execute on a plain import. Both walks share one allowlist.
"""
...
PACKAGE = "tai42_sandbox_docker"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai42_contract", "tai42_kit"})

# Every third-party root the shipped ``tai42_sandbox_docker`` graph pulls in — the
# declared runtime dependencies plus their resolved closure. Compiled extension
# modules are listed under the bare top-level name they register. Add a root here only
# for a genuine dependency of the shipped package.
ALLOWED_THIRD_PARTY = frozenset(
    {
        # The Docker Engine API client and its aiohttp transport closure.
        "aiodocker",
        "aiohttp",
        "aiohappyeyeballs",
        "aiosignal",
        ...
        "propcache",
        "yarl",
        # Shared platform substrate + pydantic settings stack (via tai42-kit).
        "_cffi_backend",
        "_openssl",
        "annotated_types",
        "anyio",
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai42_sandbox_docker graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai42_sandbox_docker module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai42_sandbox_docker sources: {offenders}"
