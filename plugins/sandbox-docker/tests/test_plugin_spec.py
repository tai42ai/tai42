...
_PACKAGED_SPEC = _REPO_ROOT / "src" / "tai42_sandbox_docker" / "tai-plugin.yml"
...
def test_plugin_spec_validates_and_names_this_listing():
    spec = _spec()
    assert spec.ref == "tai42/sandbox-docker"
...
def test_provides_is_a_single_sandbox_docker_entry():
    spec = _spec()
    assert len(spec.provides) == 1
    item = spec.provides[0]
    assert item.kind.value == "sandbox"
    assert item.name == "docker"
    assert item.module == "tai42_sandbox_docker.provider"
...
def test_pyproject_ships_the_spec_as_package_data():
    ...
    assert owners == ["tai42_sandbox_docker"], (
        "exactly the 'tai42_sandbox_docker' package must ship 'tai-plugin.yml' via "
...
def test_pyproject_ships_the_docs_tree_as_package_data():
    ...
    assert "docs/*" in package_data["tai42_sandbox_docker"], (
        "the in-package docs/ tree must ship via the 'docs/*' glob on the "
        "'tai42_sandbox_docker' [tool.setuptools.package-data] key, or the wheel omits docs/index.mdx"
    )
