...
_PACKAGED_SPEC = _REPO_ROOT / "src" / "tai42_sandbox_local" / "tai-plugin.yml"
...
def test_plugin_spec_validates_and_names_this_listing():
    spec = _spec()
    assert spec.ref == "tai42/sandbox-local"
...
def test_plugin_spec_provides_one_sandbox_local_entry():
    provides = _spec().provides
    assert len(provides) == 1
    (item,) = provides
    assert item.kind is PluginItemKind.SANDBOX
    assert item.name == "local"
    assert item.module == "tai42_sandbox_local.provider"
...
def test_descriptor_contract_range_equals_the_contract_specifier():
    project = tomllib.loads((_REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8"))["project"]
    contract_req = next(dep for dep in project["dependencies"] if dep.startswith("tai42-contract"))
    specifier = contract_req[len("tai42-contract") :]
    assert _spec().contract == specifier
...
def test_pyproject_ships_the_spec_as_package_data():
    ...
    assert owners == ["tai42_sandbox_local"], (
        "exactly the 'tai42_sandbox_local' package must ship 'tai-plugin.yml' via "
        f"[tool.setuptools.package-data]; found owners {owners!r}"
    )
...
def test_pyproject_ships_the_docs_tree_as_package_data():
    ...
    assert "docs/*" in package_data["tai42_sandbox_local"], (
        "the in-package docs/ tree must ship via the 'docs/*' glob on the "
        "'tai42_sandbox_local' [tool.setuptools.package-data] key, or the wheel omits docs/index.mdx"
    )
