...
``tai42_sandbox_local`` is on the allowlist (the shared platform substrate, its
dependency closure, and the standard library only — NO third-party runtime root of
its own, since the direct/host provider drives the host with the standard library).
The runtime walk imports the package and every submodule in a fresh subprocess and
inspects ``sys.modules``; the static walk parses every source file and collects
import roots at any nesting depth. Both walks share one allowlist.
"""
...
PACKAGE = "tai42_sandbox_local"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai42_contract", "tai42_kit"})

# Every third-party root the shipped graph pulls in — the declared runtime
# dependencies (pydantic / pydantic-settings) plus their resolved closure. The
# provider adds NO third-party root of its own (it uses only the standard library
# for host subprocesses and filesystem work).
