...
The GitHub tools register through ``tai42_app`` at import time, so a no-op app must be bound before
importing a module; individual tests rebind to a capturing or curl-backed fake.
"""
...
def curl_app() -> Iterator[None]:
    """Bind an app whose ``clients.client_ctx`` is tai42-kit's real pool, so the GitHub tools run
...
def _guard_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """Isolate the SSRF guard for the tests: clear any ambient ``TAI_URL_GUARD_*`` env and disable
    the guard by default (the GitHub calls hit a loopback stub it would block)."""
    import os
