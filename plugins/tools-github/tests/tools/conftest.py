"""Fixtures for the GitHub tool tests.
...
The ``stub_server`` and ``github_env`` fixtures stand in for the GitHub API and for the tools'
settings, so no test reaches a real GitHub host.
"""
...
class _Stub:
    ...
    current ``responder`` (default an empty 200), so a test can route by path or count."""
...
def stub_server() -> Iterator[_Stub]:
    ...
    as the GitHub API (routed by path/query/count)."""
...
@pytest.fixture
def github_env(monkeypatch: pytest.MonkeyPatch) -> Iterator[Callable[..., None]]:
    """Configure the GitHub settings through real env parsing (so empty-string cases exercise the
    same path production does) and clear the settings cache. Any ``TOOLS_GITHUB_*`` name is removed
    first so the ambient environment cannot leak in."""
    import os

    for key in list(os.environ):
        if key.startswith("TOOLS_GITHUB_"):
            monkeypatch.delenv(key, raising=False)

    def configure(*, token: str | None = "gh-token", api_base: str | None = None) -> None:
        if token is not None:
            monkeypatch.setenv("TOOLS_GITHUB_TOKEN", token)
        if api_base is not None:
            monkeypatch.setenv("TOOLS_GITHUB_API_BASE", api_base)
        reset_all_settings()
