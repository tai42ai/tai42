...
description: "Wire the Postgres accounts provider, bind its database, run migrations, and invite the team."
---
...
Postgres; login backoff lives in Redis, which arrives from the access-control
configuration.
...
The public `/api/login/*` routes: `POST /api/login/password` verifies an email and
password and mints a session, and `POST /api/login/invite/accept` sets a first password
from an invite. Passwords are at least ten characters.

The first owner is not created here. The platform's one-step setup door creates the
owner principal and, when this provider is configured, attaches the owner's login
through it — a password set now, or an invite link the owner follows later to set one.
The provider implements that attachment seam and owns no separate first-owner route.
