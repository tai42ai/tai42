...
async def _run_parallel(
    ...
    concurrent_limit = (
        max_concurrent if max_concurrent is not None else min(batch_settings().default_max_concurrent, len(params) or 1)
    )
    sem = asyncio.Semaphore(concurrent_limit)
    tasks = [asyncio.ensure_future(_run_one_body(tool_name, param, fail_fast, sem)) for param in params]
    try:
...
async def _run_sequential(
    ...
    """Run bodies one at a time, results in input order."""
    return [await _run_one_body(tool_name, param, fail_fast) for param in params]
...
async def execute_batch(
    ...
    if execution_mode == "parallel":
        results = await _run_parallel(tool_name, params, max_concurrent, fail_fast)
    elif execution_mode == "sequential":
        results = await _run_sequential(tool_name, params, fail_fast)
    else:
        raise RuntimeError(f"Unknown execution mode '{execution_mode}'")

    return _resolve_park(tool_name, results)
