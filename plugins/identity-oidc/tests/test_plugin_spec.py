...
_PACKAGED_SPEC = _REPO_ROOT / "src" / "tai42_identity_oidc" / "tai-plugin.yml"
...
def test_plugin_spec_validates_and_names_this_listing():
    spec = _spec()
    assert spec.ref == "tai42/identity-oidc"
...
def test_packaged_spec_is_declared_in_package_data():
    ...
    assert owning == ["tai42_identity_oidc"], (
...
def test_pyproject_ships_the_docs_tree_as_package_data():
    ...
    assert "docs/*" in package_data["tai42_identity_oidc"], (
        "the in-package docs/ tree must ship via the 'docs/*' glob on the "
        "'tai42_identity_oidc' [tool.setuptools.package-data] key, or the wheel omits docs/index.mdx"
    )
