"""A scheduled celery fire runs the tool inside a ``schedule`` state context built from the
...
async def test_scheduled_fire_runs_in_the_schedule_context(stub_app, monkeypatch) -> None:
    ...
    await run_tool(**{arg: "greet", SCHEDULE_SUBJECT_ARG: _SUBJECT, "message": "hi"})
    assert len(seen) == 1
    ctx = seen[0]
...
async def test_background_run_carries_no_context_even_with_a_top_level_subject(stub_app, monkeypatch) -> None:
    ...
    monkeypatch.setattr(stub_app.tools, "run_tool", _capture(seen))
    arg = celery_settings().tool_name_arg
    # A top-level ``subject`` argument with NO stamped SCHEDULE_SUBJECT_ARG is an ordinary
    # tool argument — the run stays context-free (door ``api`` at the write chokepoint).
