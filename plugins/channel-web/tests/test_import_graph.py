...
``tai42_channel_web`` is the HTTP surface, the shared platform substrate, their
dependency closure, or the stdlib — anything else fails loudly. The runtime walk
imports the package (and every submodule) in a fresh subprocess and inspects
``sys.modules``; the static walk parses every source file's full AST, catching an
import nested in a function/class/``TYPE_CHECKING`` body that the runtime walk
never executes.
"""
...
# at import time), import the package and every submodule, print each imported root
# not on the allowlist. A submodule import failure exits non-zero -> loud fail.
_CHILD_PROGRAM = f"""
import importlib
import pkgutil
import sys
...
class _StubApp:
    ...
    # attribute access yields another stub, a call with a single callable argument
    # behaves as a bare decorator, and any other call behaves as a decorator factory.
...
def _static_import_roots() -> dict[str, set[str]]:
    ...
    Walks the full AST of every source file, so an import nested inside a function
    body, a class body, or a conditional block is collected exactly like a
    module-level one. Relative imports address the shipped package itself and carry
    no root to check.
    """
