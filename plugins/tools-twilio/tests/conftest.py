"""Bind a null app to the ``tai42_app`` handle before any package import, and
provide fixtures shared across the tool and client suites.

The Twilio tools register through ``tai42_app`` at import time, so a no-op app must
be bound before importing a tool module; individual tests rebind to a capturing or
...
def curl_app() -> Iterator[None]:
    ...
    Twilio tools run against a live curl session. Restores the prior app."""
...
def local_server() -> Iterator[_LocalServer]:
    """A real localhost HTTP server on an ephemeral port for the redirect-target
    tests (the curl client cannot be intercepted by a transport-level mock)."""
...
def _guard_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    """Isolate the SSRF guard for the tests: clear any ambient ``TAI_URL_GUARD_*``
    env and disable the guard by default (the Twilio calls hit a loopback stub it
    would block)."""
    import os
