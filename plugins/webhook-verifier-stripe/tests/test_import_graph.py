...
``tai42_webhook_verifier_stripe`` is on the allowlist (tai42-contract's closure plus
...
PACKAGE = "tai42_webhook_verifier_stripe"
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai42_webhook_verifier_stripe graph failed:\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai42_webhook_verifier_stripe module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai42_webhook_verifier_stripe sources: {offenders}"
