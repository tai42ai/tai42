...
_PACKAGED_SPEC = _REPO_ROOT / "src" / "tai42_webhook_verifier_stripe" / "tai-plugin.yml"
...
def test_plugin_spec_validates_and_names_this_listing():
    spec = _spec()
    assert spec.ref == "tai42/webhook-verifier-stripe"
...
def test_packaged_spec_is_declared_in_package_data():
    ...
    assert owning == ["tai42_webhook_verifier_stripe"]
