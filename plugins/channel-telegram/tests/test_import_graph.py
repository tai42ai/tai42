...
``tai42_channel_telegram`` is the HTTP surface, the shared platform substrate,
their dependency closure, or the stdlib — anything else fails loudly. The runtime
...
PACKAGE = "tai42_channel_telegram"
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai42_channel_telegram graph failed:\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai42_channel_telegram module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai42_channel_telegram sources: {offenders}"
