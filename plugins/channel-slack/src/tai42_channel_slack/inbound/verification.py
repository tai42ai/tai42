...
class _SignatureRejectedError(Exception):
    """An ordinary request-side verification failure -> uniform 401."""


class _InboundRejectedError(Exception):
    """A transport-auth failure carrying the vendor-facing response a door returns instead of proceeding.

    The response is 413 (too large), 500 (misconfigured), or 401 (rejected).
    """
...
def _verify_signature(raw: bytes, headers: Mapping[str, str], secret: str) -> None:
    ...
    Every request-side defect raises :class:`_SignatureRejectedError`, mapped to one
    constant 401 — no oracle distinguishing the failure reason.
    """
    timestamp = headers.get(_TIMESTAMP_HEADER)
    if timestamp is None:
        raise _SignatureRejectedError(f"missing {_TIMESTAMP_HEADER} header")
    ...
    if not (timestamp.isascii() and timestamp.isdigit()):
        raise _SignatureRejectedError(f"{_TIMESTAMP_HEADER} is not an ASCII-digit integer")
    ts_value = int(timestamp)
    if abs(time.time() - ts_value) > _MAX_TIMESTAMP_SKEW_SECONDS:
        raise _SignatureRejectedError("request timestamp outside the replay window")

    signature = headers.get(_SIGNATURE_HEADER)
    if signature is None:
        raise _SignatureRejectedError(f"missing {_SIGNATURE_HEADER} header")
    if not signature.startswith(_SIGNATURE_PREFIX):
        raise _SignatureRejectedError(f"{_SIGNATURE_HEADER} is not prefixed {_SIGNATURE_PREFIX!r}")
    provided_hex = signature[len(_SIGNATURE_PREFIX) :]
    if len(provided_hex) != _HEX_DIGEST_LEN:
        raise _SignatureRejectedError(f"{_SIGNATURE_HEADER} digest is not {_HEX_DIGEST_LEN} hex characters")
    try:
        provided_digest = bytes.fromhex(provided_hex)
    except ValueError as exc:
        raise _SignatureRejectedError(f"{_SIGNATURE_HEADER} digest is not valid hex") from exc
    ...
    if not hmac.compare_digest(provided_digest, expected_digest):
        raise _SignatureRejectedError(f"{_SIGNATURE_HEADER} digest mismatch")
...
async def _read_verified_body(request: Request) -> bytes:
    ...
    unset or empty key would make the HMAC forgeable). Raises :class:`_InboundRejectedError`
    ...
    try:
        ...
        raise _InboundRejectedError(JSONResponse({"error": "payload too large"}, status_code=413)) from exc
    signing_secret = slack_settings().signing_secret
    secret = signing_secret.get_secret_value() if signing_secret is not None else ""
    if not secret:
        raise _InboundRejectedError(_misconfigured("CHANNEL_SLACK_SIGNING_SECRET"))
    try:
        _verify_signature(raw, request.headers, secret)
    except _SignatureRejectedError as exc:
        logger.warning("slack inbound door rejected: %s", exc)
        raise _InboundRejectedError(JSONResponse({"error": "signature verification failed"}, status_code=401)) from exc
    return raw
