...
from tai42_channel_slack.inbound import events, verification
...
def frozen_time(monkeypatch: pytest.MonkeyPatch) -> int:
    ...
    monkeypatch.setattr(verification, "time", SimpleNamespace(time=lambda: float(_FROZEN_NOW)))
    return _FROZEN_NOW


async def test_valid_signature_passes_to_body_logic():
    response = await events.slack_inbound(make_request(_RAW_BODY, signed_headers(_RAW_BODY, TEST_SIGNING_SECRET)))
...
async def test_fail_closed_matrix_uniform_401(case):
    ...
    response = await events.slack_inbound(make_request(_RAW_BODY, headers))
...
async def test_empty_signing_secret_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(
        verification, "slack_settings", lambda: SimpleNamespace(signing_secret=SecretStr(""), channel="C1")
    )
...
async def test_oversized_body_413_before_any_signature_work(monkeypatch):
    ...
    monkeypatch.setattr(verification, "slack_settings", _never_consulted)
    oversized = b"x" * (1 * 1024 * 1024 + 1)

    response = await events.slack_inbound(make_request(oversized, {}))
