...
:class:`~tai42_cli.client.ApiClient`. Resolution of the server URL and the
...
``config.toml`` → the local default built from :data:`DEFAULT_LOCAL_PORT`.
...
def default_server_url() -> str:
    """The local server URL built from :data:`DEFAULT_LOCAL_PORT`."""
    return f"http://127.0.0.1:{DEFAULT_LOCAL_PORT}"
