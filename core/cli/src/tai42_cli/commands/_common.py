...
Every ``tai`` remote command is a thin wrapper over one or more of the server's
...
def parse_assignment_arg(token: str, *, param_hint: str) -> tuple[str, str]:
    """One ``KEY=VALUE`` command-line token, raising a usage error otherwise."""
    pair = _split_assignment(token)
    if pair is None:
        raise typer.BadParameter(f"expected KEY=VALUE, got {token!r}", param_hint=param_hint)
    return pair


def parse_env_lines(text: str, *, source: str) -> list[tuple[str, str]]:
    """Parse ``KEY=VALUE`` lines from ``text`` (see :func:`_split_assignment`).

    Blank lines and lines whose first non-space character is ``#`` are skipped; any
    other line that is not ``KEY=VALUE`` raises a usage error naming ``source`` and
    the 1-based line number ONLY — never the line content, which may itself be a
    secret. No quote processing — the value is taken verbatim.
    """
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if not stripped or stripped.startswith("#"):
            continue
        pair = _split_assignment(line)
        if pair is None:
            raise typer.BadParameter(f"{source} line {line_number}: expected KEY=VALUE with a non-empty key")
        pairs.append(pair)
    return pairs
...
def read_file_or_stdin(source: str, *, param_hint: str) -> str:
    """Read the whole text of ``source``, or of stdin when ``source`` is ``-``.

    Keeps a secret value off the command line (a value on argv leaks via ``ps`` and
    shell history). A missing/unreadable file raises a usage error naming the option.
    """
    if source == "-":
    ...
    try:
        ...
        raise typer.BadParameter(f"cannot read {source!r}: {exc}", param_hint=param_hint) from exc


def load_json_object_arg(
    inline: str | None,
    file: str | None,
    *,
    param_hint: str,
    ...
    """Resolve a JSON object from an inline string OR a file/stdin (``-``) source.

    Reading the value from a file keeps a secret off the command line (a value on
    argv leaks via ``ps`` and shell history). Giving both is a usage error; giving
    ...
    if inline is not None:
        return parse_json_object(inline, param_hint=param_hint)
    if file is not None:
        return parse_json_object(read_file_or_stdin(file, param_hint=file_param_hint), param_hint=file_param_hint)
    return None
