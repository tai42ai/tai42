...
class FolderNotFoundError(ToolMetaError):
    ...
    def __init__(self, folder_id: str):
        """Store the ``folder_id`` the missing folder was addressed by."""
        self.folder_id = folder_id
        super().__init__(f"folder {folder_id!r} not found")


class FolderNotEmptyError(ToolMetaError):
    """A folder delete was refused because the folder still holds subfolders or tool-meta rows.

    A folder is deletable only when empty of both.
    """
    ...
    def __init__(self, folder_id: str):
        """Store the ``folder_id`` of the non-empty folder."""
        self.folder_id = folder_id
        super().__init__(f"folder {folder_id!r} is not empty")


class FolderCycleError(ToolMetaError):
    """A folder MOVE was refused because it would make ``folder_id`` its own ancestor (a parent-chain cycle).

    This is the move-time guard only: creation just validates that the parent exists — a freshly created
    folder's id cannot already sit in any parent chain, so a create can never form a cycle.
    """
    ...
    def __init__(self, folder_id: str):
        """Store the ``folder_id`` whose move would form a cycle."""
        self.folder_id = folder_id
        super().__init__(f"moving folder {folder_id!r} there would create a cycle")


class FolderNameConflictError(ToolMetaError):
    """A folder ``name`` already exists under the same ``parent_id`` (``None`` for root folders).
    ...
    def __init__(self, name: str, parent_id: str | None):
        """Store the conflicting ``name`` and the ``parent_id`` it collides under (``None`` at the root)."""
        self.name = name
