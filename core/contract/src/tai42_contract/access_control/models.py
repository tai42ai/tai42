...
class AccessPolicy(ConditionMixin):
    ...
    condition: Annotated[TemplatedText | None, Field(json_schema_extra=_ACCESS_CONDITION_ANNOTATION)] = None
...
class RoleDefinition(ConditionMixin):
    ...
    # their own `condition` directly. The `condition` field (the jq base) comes from
    # ConditionMixin; it is redeclared below only to carry the STRICT-TRUE annotation.
