...
def _write_target() -> dict[str, Callable[..., IdentityProvider]]:
    """Return where a registration lands: the staged generation while staging, else the committed registry.

    Boot writes straight to the empty committed map. Safe without thread-locals: the reload gate serialises
    builds one at a time and nothing on the serving path registers.
    """
    return _pending if _pending is not None else _REGISTRY


def same_factory(existing: object, factory: object) -> bool:
    """Whether two provider factories denote the SAME provider — the reload-safety predicate.

    Shared by both this registry and the accounts registry. True when the factories are the same object,
    OR when they share ``__module__``
...
def register_identity_provider(name: str, factory: Callable[..., IdentityProvider]) -> None:
    ...
    RELOAD-SAFE: re-registering the SAME factory (by :func:`same_factory`) under a name it already holds
    is a quiet no-op, so the hot-reload primitive re-executing a plugin's module body does not raise;
    a DIFFERENT factory under an already-registered name still raises loudly.
    """
...
def reset_registry() -> None:
    ...
    While a build is staging, ``start()`` clears the fresh staged map before it re-imports the plugin
    modules, never the committed one, so a reload leaves the live registry untouched; otherwise the
    committed map is cleared (boot, and test isolation).
    """
    _write_target().clear()


def begin_staging() -> None:
    """Open a fresh staged generation an epoch build registers into.

    Leaves the committed registry serving the live generation untouched.
    """
    global _pending
    _pending = {}
...
def abort_staging() -> None:
    """Drop the staged generation on a failed build — the committed registry never saw any of its registrations."""
    global _pending
    _pending = None
