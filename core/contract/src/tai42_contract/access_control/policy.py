"""The ``PolicyEnforcer`` protocol: fetch a user's policy plus live context and enforce a jq expression.

Implementations add redis fetch + lru caching.
"""
...
class PolicyEnforcer(Protocol):
    """Protocol for fetching a user's access policy plus live context and enforcing jq conditions."""
    ...
    async def get_live_context(self, user_id: str) -> dict[str, Any]:
        ...
        Enforcement is therefore based on live data.
        """
        ...
    ...
    def enforce(self, context: dict[str, Any], expression: str | None, *, condition_configured: bool = False) -> None:
        """Evaluate the jq ``expression`` against ``context``; raise on violation (never silently allow).
