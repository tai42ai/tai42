...
class VersionedStore(Protocol):
    ...
    async def create(
        ...
        *,
        ...
        """Insert a new document at ``active_version = 1`` plus its version 1, one transaction.

        Raise :class:`DocumentExistsError` if ``(kind, name)`` is
    ...
    async def save_version(
        ...
        *,
        ...
        """Append a new version and bump the active pointer to it, one transaction.

        ``new_version = max(version) + 1``, NOT ``active_version + 1``. A partial failure rolls
        the whole save back — no orphan version, no half-bumped pointer. Raise
        :class:`DocumentNotFoundError` if absent. Runs within ``tx`` when one is supplied.
        """
        ...
    ...
    async def get(self, kind: str, name: str) -> DocumentRecord:
        """Fetch the active record for ``(kind, name)``.

        Raise :class:`DocumentNotFoundError` if absent.
        """
        ...
    ...
    async def get_active_body(
        ...
        *,
        ...
        """Return the ``body`` of the active version for ``(kind, name)``.
    ...
    async def list_versions(self, kind: str, name: str) -> list[DocumentVersion]:
        """List every version of ``(kind, name)`` with its :attr:`DocumentVersion.is_current` signal.

        ``is_current`` is derived from ``active_version``. Raise :class:`DocumentNotFoundError` if the
        document is absent.
        """
        ...
    ...
    async def get_version(
        ...
        Raise :class:`DocumentVersionNotFoundError` if that
    ...
    async def rollback(
        ...
        """Re-point ``active_version`` to ``version`` (NO data copy).

        Raise :class:`DocumentVersionNotFoundError` if that version does not exist. Runs
        within ``tx`` when one is supplied.
        """
        ...

    async def soft_delete(self, kind: str, name: str) -> None:
        """Set ``is_active = False``, keeping the version history (audit).

        The document drops out of ``list``/``get`` but its rows survive.
        """
        ...

    async def delete(self, kind: str, name: str, *, tx: VersionedStoreTransaction | None = None) -> None:
        """HARD delete, DISTINCT from :meth:`soft_delete`.

        Remove ONLY the ACTIVE
    ...
    async def rename(self, kind: str, name: str, new_name: str) -> DocumentRecord:
        ...
        The whole version history, every per-version ``tags`` label, and
