    ...
    return StateBinding(
        states=[
            StateAttach(
                state="status",
                templates=["planner"],
                subject_expr=".thread_id",
                scope_expr='{target_kind: "agent", target_name: .our_identity}',
                input_injections=[
                    StateInjection(template_jq="bound", into="context"),
                    StateInjection(jq="{record: .record, n: (.input.count)}", into="extra"),
                ],
                updates=[
                    StateUpdate(template_jq="mark", adapter="{verdict: .output.status}", op_id=".input.turn"),
...
def test_update_sets_exactly_one_source_and_custom_has_no_adapter() -> None:
    with pytest.raises(ValidationError, match="exactly one"):
        StateUpdate(template_jq="a", jq=".")
    with pytest.raises(ValidationError, match="no 'adapter'"):
        StateUpdate(jq="[]", adapter="{x: .output}")


def test_attach_refuses_a_bad_state_name_and_requires_a_subject() -> None:
    with pytest.raises(ValidationError):
        StateAttach(state="Bad Name", subject_expr=".id")
    with pytest.raises(ValidationError):
        StateAttach(state="status", subject_expr="")
