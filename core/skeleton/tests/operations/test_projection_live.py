"""LIVE-stack verification of the MCP projection.
...
# -- the projected surface is exactly the 166 default ops ---------


def test_projected_surface_is_the_expected_op_count():
    async def run():
...
# -- destructiveHint on destructive ops, absent on reads ----------


def test_destructive_hint_present_on_mutations_absent_on_reads():
    async def run():
        async with app.app_context(_manifest()):
            live = await app.tools.get_tools()
...
# -- the duplicate-bind collision guard fires at boot -------------


def test_duplicate_bind_of_builtin_and_projection_fails_boot():
...
# -- tier-2 absent by default, includable ------------------------


def test_tier2_absent_by_default_but_projects_when_included():
    async def run():
...
# -- tier-1 hardcode-blocked even when included -------------------


def test_tier1_blocked_even_when_explicitly_included(caplog):
    async def run():
...
# -- get_me (caller-context identity op) is tier-1, never projectable --


def test_get_me_is_caller_context_tier1_never_projectable(caplog):
...
# -- user_tools curation coexists with api_tools -----------------


def test_user_tools_curation_coexists_with_api_tools():
    ...
    async def run():
        ...
        async with app.app_context(manifest):
            ...
            # return the empty persisted view. The curation verified here is the live one.
