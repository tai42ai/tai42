...
1. the projected tool surface is exactly the expected op surface — the 118
   default-projected ops (160 total - 38 tier-2 default-excluded - 4 tier-1
   hardcode-blocked);
...
# -- checklist 1: the projected surface is exactly the 118 default ops ---------
...
def test_d1_projected_surface_is_the_expected_op_count():
    async def run():
        async with app.app_context(_manifest()):
            ...
            # The arithmetic: 160 total - 38 tier-2 default-excluded - 4 tier-1 hardcode-blocked = 118.
            ...
            # it stays on the default surface, like ``upsert_tool_meta``).
            ...
            # The default-projected surface = 118, measured two ways.
            recorder = _RecordingApp()
            projected = project_operations(recorder, ApiToolsConfig(), registry=reg)
            assert len(projected) == 118, len(projected)
            assert total - len(tier2) - len(tier1) == 118

            # And the LIVE booted tool surface is exactly those 118 (no keep-set /
