...
1. the projected tool surface is exactly the expected op surface — the 119
   default-projected ops (161 total - 38 tier-2 default-excluded - 4 tier-1
   hardcode-blocked);
...
# -- checklist 1: the projected surface is exactly the 119 default ops ---------
...
def test_d1_projected_surface_is_the_expected_op_count():
    async def run():
        async with app.app_context(_manifest()):
            ...
            # The arithmetic: 161 total - 38 tier-2 default-excluded - 4 tier-1 hardcode-blocked = 119.
            ...
            # set/unset booleans only), a tier-0 read like its ``get_manifest`` siblings.
            ...
            # The default-projected surface = 119, measured two ways.
            recorder = _RecordingApp()
            projected = project_operations(recorder, ApiToolsConfig(), registry=reg)
            assert len(projected) == 119, len(projected)
            assert total - len(tier2) - len(tier1) == 119

            # And the LIVE booted tool surface is exactly those 119 (no keep-set /
