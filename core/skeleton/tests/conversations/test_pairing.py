"""The multichannel pairing turn: classification inside accept, link/redeem/unlink, the
first-contact greeting, and the redeem brute-force throttle."""
...
async def test_invalid_code_is_an_answered_uniform_reply(env, monkeypatch):
    ...
    assert record.answer == pairing_module._INVALID_CODE_TEXT
...
async def test_a_locked_source_does_not_burn_a_valid_code_and_leaks_no_oracle(env, monkeypatch):
    ...
    assert locked_reply == pairing_module._INVALID_CODE_TEXT
    assert set(replies) == {pairing_module._INVALID_CODE_TEXT}  # no oracle: every reply identical
