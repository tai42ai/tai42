...
def test_set_get_list_delete_round_trip(client):
    ...
    deleted = client.delete("/api/conversation-configs/agent/concierge")
...
def test_a_body_key_agreeing_with_the_path_takes_the_success_path(client):
    resp = client.put(
        "/api/conversation-configs/agent/concierge",
        json={"target_kind": "agent", "target_name": "concierge", "multichannel": True},
    )
...
def test_a_non_object_body_is_a_400(client):
    resp = client.put("/api/conversation-configs/agent/concierge", json=["not", "an", "object"])
    assert resp.status_code == 400


def test_an_unknown_placeholder_is_a_400(client):
    resp = client.put("/api/conversation-configs/agent/concierge", json={"greeting_template": "hi {name}"})
    assert resp.status_code == 400
...
def test_get_unknown_is_a_404(client):
    resp = client.get("/api/conversation-configs/agent/concierge")
    assert resp.status_code == 404


def test_delete_unknown_is_a_404(client):
    resp = client.delete("/api/conversation-configs/agent/concierge")
    assert resp.status_code == 404
