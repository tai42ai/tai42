"""The channels router's auth boundary, pinned with access control ENABLED.

``GET /api/channels`` enumerates the deployment's installed delivery channels —
deployment-topology information, so it is AUTHED. This asserts the catalog read
is rejected with no credential — the handler never runs.
"""
...
import tai42_skeleton.routers.channels as router
...
_PATH_PATTERNS = {r"/api/channels": "channels-api"}
...
def _boundary_client(monkeypatch) -> TestClient:
    ...
    ac_fake = _AcFake({"channels-api": "channels-api-protected"})
    ...
    routes = [Route("/api/channels", router.list_channels, methods=["GET"])]
    app = Starlette(routes=routes, middleware=AuthAdapter(ac_settings).get_middleware())
    return TestClient(app)


def test_list_channels_rejected_without_auth(monkeypatch):
