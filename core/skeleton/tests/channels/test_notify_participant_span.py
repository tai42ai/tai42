...
from tai42_skeleton.channels.inbound import InboundBridge, _notify_participant
...
async def test_notice_sends_and_span_no_ops_without_a_trace(backend, register_channel):
    ...
    await _notify_participant(_bridge(), "your answer could not be accepted")
...
async def test_notify_failure_is_swallowed_and_recorded_under_a_trace(backend, register_channel):
    ...
    await _notify_participant(_bridge(), "closed")
