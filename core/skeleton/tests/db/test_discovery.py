...
def test_plugin_entry_none_when_no_migrations_declared() -> None:
    assert plugin_migration_entry(_spec(package="tai42-skeleton", migrations=None), SchemaAdminSettings()) is None
...
def test_plugin_entry_resolves_package_relative_directory() -> None:
    ...
    entry = plugin_migration_entry(spec, SchemaAdminSettings())
    assert entry is not None
    assert entry.component == "tai42-skeleton"  # component identity is the distribution
    assert entry.migrations_dir.is_dir()


async def test_all_entries_skeleton_only_without_marketplace_store() -> None:
    # No ``MARKETPLACE_STORE_*`` in the test env → no installed-plugin chains.
    entries = await all_migration_entries(SchemaAdminSettings())
...
async def test_installed_plugin_entries_reads_marketplace_store(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from tai42_skeleton.marketplace import settings as mp_settings
    ...
    monkeypatch.setattr(mp_settings, "marketplace_store_configured", lambda: True)
    monkeypatch.setattr(mp_store, "MarketplaceInstallStore", _FakeStore)

    entries = await installed_plugin_entries(SchemaAdminSettings())
