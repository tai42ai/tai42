"""Universal-authz sweep — the core LIVE-stack security proof.
...
_SWEEP_PRESET = "authz_sweep_preset"  # preset baked over remove_tool
_MAIN_MODALITIES = [_A_PLAIN, _A_PRIV, _B_BACKEND, _C_INPROC, _SWEEP_PRESET]
...
async def _register_preset() -> None:
    ...
    await app.preset_manager.register(_SWEEP_PRESET, _A_PLAIN, {}, [], "")
...
def _tear_down_sweep_preset():
    ...
    if manager.is_registered(_SWEEP_PRESET):
        asyncio.run(manager.remove(_SWEEP_PRESET))
...
def test_authz_sweep_deny_covers_every_default_projected_op(monkeypatch: pytest.MonkeyPatch):
...
def test_authz_sweep_deny_with_insufficient_scope_over_every_modality(monkeypatch: pytest.MonkeyPatch):
...
def test_authz_sweep_allow_with_sufficient_identity_over_every_modality(monkeypatch: pytest.MonkeyPatch):
...
def test_authz_sweep_allow_when_access_control_disabled(monkeypatch: pytest.MonkeyPatch):
...
def test_authz_sweep_sub_mcp_mount_deny_and_allow(monkeypatch: pytest.MonkeyPatch):
    ...
    async def run():
        async with app.app_context(_sweep_manifest()):
            ...
            async with router.lifespan(cast("Any", None)):
                await router.register_sub_mcp_app("authzsweep", [_A_PLAIN], transport="http")
                await router._get_or_build_app("authzsweep")
...
# -- owned-key OWNER-attenuation parity on the MCP edge -------------


def test_owned_key_attenuation_holds_on_the_mcp_dispatch_edge(monkeypatch: pytest.MonkeyPatch):
