"""The state-module seed registry and the seed applier — the module-side twin of the preset
...
from tai42_skeleton.states.seeds import StateModuleSeedRegistry, apply_module_seeds
...
class _FakeSeedStore:
    ...
    async def get_module(self, name: str):
...
def test_registry_registers_and_lists() -> None:
    reg = StateModuleSeedRegistry()
    a = StateModuleDocument(name="a")
    b = StateModuleDocument(name="b")
    reg.register(a)
    reg.register(b)
...
def test_registry_refuses_duplicate_name() -> None:
    reg = StateModuleSeedRegistry()
    reg.register(StateModuleDocument(name="a"))
...
async def test_apply_seeds_creates_absent_and_stamps_hash() -> None:
    ...
    doc = StateModuleDocument(name="shipped")
    await apply_module_seeds(store, seeds=[doc])  # type: ignore[arg-type]
...
async def test_apply_seeds_leaves_present_untouched() -> None:
    ...
    await apply_module_seeds(store, seeds=[StateModuleDocument(name="already")])  # type: ignore[arg-type]
    assert store.upserts == []  # idempotent — a present name is skipped


async def test_apply_seeds_hash_is_content_stable() -> None:
    doc = StateModuleDocument(name="m")
    s1, s2 = _FakeSeedStore(), _FakeSeedStore()
    await apply_module_seeds(s1, seeds=[doc])  # type: ignore[arg-type]
    await apply_module_seeds(s2, seeds=[doc])  # type: ignore[arg-type]
