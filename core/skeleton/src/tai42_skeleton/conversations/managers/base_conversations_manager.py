...
class DoorFlipRefusedError(Exception):
    """Raised by :meth:`BaseConversationsManager.put_route` when a door flip would strand held threads.

    The write would change the ``door`` of a route that already holds threads.
    Carries what the refusing step read, so the caller's operator-facing message
    names the state the write was actually refused against and not a count read a
    round trip earlier.
    """

    def __init__(self, route_name: str, from_door: str, to_door: str, held: int) -> None:
        """Record the route, the ``from``/``to`` doors, and the held-thread count read at refusal."""
...
class BaseConversationsManager(ABC):
    """The routing-row store — keyspace 4 of the conversation bridge.
    ...
    async def put_route(self, route: ConversationRoute) -> bool:
        """Store ``route`` (an upsert — create or replace), keeping the name index in lockstep.

        Return ``True`` when the row is newly created, ``False`` when it replaced an
        existing row of the same name.

        Raise :class:`DoorFlipRefusedError` — writing nothing — when the row already stored
    ...
    async def get_route(self, route_name: str) -> ConversationRoute | None:
        """The stored row for ``route_name``, or ``None`` when no such route exists.

        Its ``callback_secret`` is included, for the delivery executor.
        """
        ...

    @abstractmethod
    async def delete_route(self, route_name: str) -> bool:
        """Remove the row for ``route_name``, keeping the name index in lockstep.
