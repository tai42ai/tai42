...
def _now() -> float:
    """Return the monotonic clock the caps read — one seam, so refill and cache expiry advance together."""
    return time.monotonic()


class ThreadQueueOverflowError(UnavailableError):
    """The per-thread FIFO is full — a loud, retriable 503 rather than an unbounded backlog."""


class ThreadBusyError(UnavailableError):
    """A live-caller sync door's bounded wait to acquire the per-thread turn slot elapsed.

    An in-flight — possibly HITL-paused on another worker — turn still held it: a loud,
...
class AddressRateLimitedError(UnavailableError):
    """A bucket key is over its per-hour turn cap on a synchronous door.

    A loud, retriable refusal instead of the channel door's paid slow-down reply.
    """


class AddressAdmission(Enum):
    """The token bucket's verdict for one bucket key.

    Run the turn, run none but owe one slow-down reply (first over-limit hit of a window), or
    run none and send nothing.
    """
...
class _TokenBucket:
    """One bucket key's token bucket plus its slow-down-reply cooldown.

    Capacity is one hour's worth of turns; it refills continuously at the per-hour rate.
    """
    ...
    def _rerate(self, per_hour: int) -> None:
        """Re-rate this bucket to ``per_hour`` IN PLACE.

        Never hand back tokens (they are only ever clamped down) and keep the refill clock,
        so no in-flight refill is corrupted. ``capacity`` doubles as the bucket's current rate.
        """
        self.capacity = float(per_hour)
        self.refill_per_second = per_hour / 3600.0
...
class _ConcurrencyGate:
    """The global in-flight-turn ceiling, admitting waiters in arrival order.

    The limit is re-set in place rather than by rebuilding the gate, so a raised or lowered
    ceiling counts the turns already running against it instead of granting them a second one.
    """
...
class TurnCaps:
    ...
    def admit_address(self, bucket_key: str, per_hour: int | None = None) -> AddressAdmission:
        ...
        If not, the verdict says whether this hit still owes a paid slow-down reply. The key
        is opaque here: each door composes one naming the party its cap holds accountable.
    ...
    def reserve_thread_slot(self, thread_id: str) -> None:
        """Reserve a place in ``thread_id``'s FIFO, raising :class:`ThreadQueueOverflowError` at ``thread_queue_depth``.

        Synchronous, so an overflow is refused before any state is written for the message.
        Every reservation must be released by exactly one :meth:`run_reserved` or
    ...
    async def run_reserved(self, thread_id: str, *, acquire_timeout_seconds: float | None = None):
        """Run a turn holding ``thread_id``'s FIFO lock, its cross-worker lease and a global concurrency slot.

        Releases the reservation :meth:`reserve_thread_slot` took when the body exits.
    ...
    def release_thread_slot(self, thread_id: str) -> None:
        """Give back one reservation :meth:`reserve_thread_slot` took, so an aborted accept leaves the FIFO intact."""
...
def get_turn_caps() -> TurnCaps:
    """The per-worker turn caps, built from ``CONVERSATIONS_*`` config and held for the process.

    New bounds from a settings reload are applied to the SAME instance.
    """
    global _CAPS_STALE
    with _CAPS_LOCK:
