...
An entry with no ``part`` field reads as part 0, the single-part answer's only part. It
also carries the provider ids that chunk produced.
...
class LedgerInconsistentError(RuntimeError):
    """A ledger that cannot describe the answer — an unparseable entry, or totals that contradict the answer.

    Corrupt state a re-drive can never resume from, distinct from a transient store fault, which propagates untouched.
    """


@dataclass(frozen=True)
class SentChunk:
    """One chunk a channel provider has already accepted.

    Records how many characters it carried, the outbound ids the provider assigned it, and which answer PART it
    belongs to. ``part`` defaults to 0 — a single-part answer's only part, and the value read for a stored entry
    that carries no ``part`` field.
    """
...
def _decode(entry: bytes | str) -> SentChunk:
    """Parse one stored ledger entry.

    A malformed entry raises :class:`LedgerInconsistentError` rather than reading as "nothing sent", which would
    re-send chunks the provider already accepted. A ``part`` absent from the entry reads as
    0 via ``payload.get("part", 0)`` — the single-part answer's only part.
    """
    try:
...
class ChannelSendLedger:
    """The append-only per-record channel send ledger (keyspace 5).

    Construction refuses without the redis conversations backend — progress lost with the process could only be
    resumed by re-sending from the first chunk.
    """

    def __init__(self, settings: ConversationsSettings) -> None:
        """Bind to ``settings``; refuse an in-memory backend, which cannot persist send progress."""
        if settings.in_memory:
            raise NotSupportedError(_NO_BACKEND)
        self.settings = settings

    async def sent_chunks(self, message_id: str) -> list[SentChunk]:
        """Every chunk already accepted for ``message_id``, in send order.

        Empty for a send that has not started, and for one whose ledger has been cleared.
        """
    ...
    async def append(self, message_id: str, chars: int, outbound_ids: list[str], part: int = 0) -> None:
        """Record that a chunk of ``chars`` characters of answer ``part`` was accepted, producing ``outbound_ids``.

        MUST be called before the next chunk is sent, so at most the one chunk in flight can ever be re-sent.
        ``part`` defaults to 0 — a single-part answer's only part.
        """
    ...
    async def clear(self, message_id: str) -> None:
        """Drop the ledger for a record whose send is over; the record itself now carries the full outbound id list."""
        async with client_ctx(RedisClient, self.settings.redis) as r:
