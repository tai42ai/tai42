...
class UnauthenticatedApiCallerError(NotSupportedError):
    """The API door was reached with no authenticated caller principal, so the turn is refused.
...
class ThreadNotFoundError(LookupError):
    """The event door named a thread that does not exist on the route.

    An event enters an EXISTING thread as a turn and never mints one, so an unknown (route, address) or
    thread id is refused rather than opening a new conversation.
    """
...
class OperatorAppendError(RuntimeError):
    """Appending an operator's message to the thread's agent checkpoint failed, so the send is refused.

    NO record is created — the operator's reply must not stand in the transcript while it is absent from the
    memory a later agent turn reads.
    """


class CompletionDeliveryError(RuntimeError):
    """The resumed answer of an async-parked agent turn could not be delivered.

    Its thread could not be reversed to a live route + address. Raised loudly so the completion
