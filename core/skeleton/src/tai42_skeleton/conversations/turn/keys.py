...
def _api_client_address(caller_principal: str, address: str) -> str:
    """The API door's address slot: its authenticated caller joined to the caller-supplied end-user id.

    The principal is percent-encoded, so it holds no ``/`` and the join is
...
def _channel_bucket_key(route_name: str, cap_key: str) -> str:
    """The channel door's rate-bucket key.

    ``cap_key`` is the party the door named as accountable — a provider-attested
    address, or a self-minting door's network client bucket — and the route scopes it
    so two routes never share a budget.
    """
    return f"{route_name}|{cap_key}"


def _api_bucket_key(route_name: str, caller_principal: str) -> str:
    """The API door's rate-bucket key.

    It is the authenticated CALLER, not the composed address, whose cardinality the
...
def _throttle_source_key(door: ConversationDoor, accountable: str) -> str:
    """The redeem-throttle SOURCE scope: the DOOR-QUALIFIED accountable party.

    Never the conversation address (whose cardinality the caller freely chooses).
    Same accountability model the rate caps use — the api door keys on its
    authenticated ``caller_principal`` (:func:`_api_bucket_key`), the channel door on
    the provider-attested ``cap_key`` (:func:`_channel_bucket_key``). A deterministic
    JSON array (no delimiter joining) whose leading door element keeps an api
    principal string and a channel value from ever colliding, and whose accountable
    part an attacker cannot rotate per attempt — so the lock actually arms against a
    brute-force run.
    """
