"""The per-thread conversation-mode store — keyspace 18 of the conversation bridge.

The ``thread_id`` → ``agent``/``manual`` override map, transient runtime state (NOT a
backup section) and Redis-backed.
...
def _validate_mode(mode: str) -> ConversationMode:
    """Return ``mode`` as a :data:`ConversationMode`, or raise on any other value.

    A mode is never coerced or defaulted.
    """
...
class ConversationModeStore:
    """The Redis-backed per-thread mode-override store (keyspace 18).
    ...
    async def get_mode(self, thread_id: str) -> ConversationMode | None:
        """The stored override for ``thread_id``, or ``None`` when none is set.

        A stored value that is not a known mode is corruption and raises.
        """
...
async def effective_mode(route: ConversationRoute, thread_id: str) -> ConversationMode:
    """The mode in force for ``thread_id`` on ``route``.
...
async def default_mode(route: ConversationRoute, thread_id: str) -> ConversationMode:
    """The no-override default for ``thread_id``.

    A route-keyed thread takes its route's ``initial_mode``. A LINKED person's aggregated
    thread (``bridge:@person:`` prefix) spans several routes and one operator control
    decision covers them all: it defaults to ``manual`` when ANY route the person's
    addresses span defaults to ``manual``, so a linked person is never silently
    agent-served on a channel an operator meant to hold by hand, else ``agent``. A person
    row that has vanished falls back to the named route's default.
    """
...
async def default_mode_for_routes(manager: BaseConversationsManager, route_names: Iterable[str]) -> ConversationMode:
    """``manual`` when ANY of ``route_names`` resolves to a route defaulting to ``manual``, else ``agent``.

    The one place the several-routes fold lives. A name that no longer resolves imposes
    nothing, so a deleted route never forces manual.
    """
...
def supports_thread_append(agent: Agent) -> bool:
    """Whether ``agent`` implements :meth:`Agent.append_thread_messages` rather than the ABC default.

    A memoryless target (the default) has no thread memory to feed, so a manual-mode
    inbound and an operator send skip the append and record silently; a memory-holding
    target has the message appended for a later agent turn to read.
    """
    return type(agent).append_thread_messages is not Agent.append_thread_messages


class NoBridgeTurnError(RuntimeError):
    """``set_conversation_mode`` was called with no bridge turn in flight, so there is no conversation.

    There is no current conversation whose mode to set — a builtin used outside an agent
    turn refuses loudly rather than acting on nothing. Programmatic callers set a
    thread's mode through the ``PUT /thread/mode`` door instead, which names its thread
    explicitly.
    """


async def set_current_thread_mode(mode: str) -> ConversationMode:
    """Set the CURRENT bridge turn's thread mode to ``mode``, returning the stored value.

    The feature body behind the ``set_conversation_mode`` builtin tool.
