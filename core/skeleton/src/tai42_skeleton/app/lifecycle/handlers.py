...
class LifecycleHandlersMixin(LifecycleState):
    ...
    def _on_post_swap(self, func: Callable):
        """Register a handler that establishes a loop-affine background loop on the serving loop.

        Run at boot and after every epoch swap, never on the throwaway build-thread loop the
        per-epoch handlers run on.
        """
    ...
    async def _run_tool_reload(self, kind: str, action: str, name: str) -> dict[str, Any]:
        """Apply one tool reload/remove via the registered reloader.

        Raises on an unknown kind/action or a failing reloader.
        """
    ...
    async def _run_handlers(self, handlers: list[Callable], raise_on_error: bool = False):
        """Run lifecycle handlers, always attempting every handler.

        The shutdown path swallows-and-logs so teardown reaches every handler; the startup and
        reload paths pass ``raise_on_error`` so a failed handler surfaces loudly instead of leaving
        a healthy-looking half-initialized app or reporting a successful reload with missing tools.
        """
        errors: list[tuple] = []
    ...
    def _epoch_handlers(self) -> list[Callable]:
        ...
        De-duplicated by qualified name so a handler registered on BOTH hooks (the
        presets/sub-MCP/studio rehydration set) runs exactly once per epoch build. First-occurrence
        order is preserved, so the startup ordering — including presets-before-sub-MCP — holds; the
        reload-only handlers append after. Every startup handler runs per epoch (eager provider init
        included), so a rebuilt epoch's providers are instantiated before its first request.
        """
