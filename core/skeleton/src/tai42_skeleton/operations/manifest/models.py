...
class McpConfigUpdate(BaseModel):
    """A replacement MCP config section.

    The full list of MCP server entries that overwrites the manifest's ``mcp``
    list before the reload.
    """
...
class McpEntriesAdd(BaseModel):
    """MCP entries to append to the manifest's ``mcp`` section.

    ``replace`` lets an entry whose title already exists swap in at its current
    position; without it a title collision is refused.
    """
...
class ToolsEntriesAdd(BaseModel):
    """Tools entries to append to the manifest's ``tools`` section.
...
class AgentsEntriesAdd(BaseModel):
    """Agents entries to append to the manifest's ``agents`` section.
...
class ApiToolsListsUpdate(BaseModel):
    """Names to add to / remove from the manifest ``api_tools`` include and exclude lists.

    Each field is a bare name list; all four empty is refused.
    """
...
class McpTargets(BaseModel):
    """An optional fleet fan-out restriction.

    A ``targets`` list naming the workers a single-server MCP action applies to
    (all workers when omitted).
    """
...
class FailedMcpsQuery(BaseModel):
    """The failed-MCP listing door's ``?targets=`` fan-out restriction, published as an optional array.

    A repeated ``?targets=`` param a generated client sends once per worker (all
    workers when none is given).
...
class ManifestReplace(BaseModel):
    """A full-manifest replacement carrying the manifest TEXT verbatim.

    The PRESERVED view (``!ENV`` markers intact). The server loads it to the
    preserved document and persists it through the pipeline, so it owns resolution
    and no secret bakes to disk. There is no ``targets``: a persisted replacement
