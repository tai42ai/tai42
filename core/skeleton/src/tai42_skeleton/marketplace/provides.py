"""Classify a spec's ``provides`` items by their :data:`KIND_MANIFEST_BINDINGS` mode and manifest pointer.

Pure spec introspection — no I/O, no app handle — so a caller can ask which items
...
def has_manifest_provides(spec: PluginSpec) -> bool:
    """Whether the spec provides any item that wires into a manifest field (a non-env-selected item).
...
def env_selected_items(spec: PluginSpec) -> list[PluginItem]:
    """The spec's provides items that wire into no manifest field (the env-selected ``config`` kind).

    Pip install/uninstall is their whole registration.
    """
...
def mcp_entry_items(spec: PluginSpec) -> list[PluginItem]:
    """The spec's provides items that write an ``mcp`` entry (an mcp-server item carrying transport config, no module).

    A spec never mixes mcp-server with any other kind, so a non-empty result means this
    whole install is an mcp-server one.
    """
    items: list[PluginItem] = []
    for item in spec.provides:
        binding = KIND_MANIFEST_BINDINGS.get(item.kind)
...
def env_manifest_pointer(spec: PluginSpec) -> str:
    """The manifest field(s) the spec's DATA items bind into (``mcp`` / ``connectors``), joined.

    The pointer named in a combined-op orphan report. A spec never mixes mcp-server with
    other kinds and every connector item binds to ``connectors``, so this is a single field
    in practice; ``manifest`` when the spec has no data item (never on the env-accepting
    path).
    """
    fields: list[str] = []
    for item in spec.provides:
