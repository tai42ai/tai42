...
def eval_script(client: AsyncRedis, script: str, numkeys: int, *keys_and_args: object) -> Awaitable[Any]:
    """``client.eval(script, numkeys, *keys_and_args)`` accepting redis's real ``EncodableT`` arguments.

    Accepts ``bytes``/``int``/``str`` arguments, which the async stub narrows to ``str``.
    Returns the async client's true awaitable result.
    """
