...
-- marketplace install-attribution record, the tool-metadata overlay, and the
-- platform-side runs index. One transaction (the runner wraps each file); every
-- object guards with IF NOT EXISTS / ON CONFLICT so a replay is inert.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS marketplace_installs (
    ...
    -- The artifact channel the version was pinned from. `spec` is a descriptor-only
    -- plugin (a PluginSpec whose every item is a data item and which names no
    -- `package`): it resolves from a `spec` pointer and installs nothing but its
    -- manifest entry, carrying its provenance in the same repository_url/tag/
    -- artifact_ref/sha256 columns a github pin uses.
    source         TEXT         NOT NULL CHECK (source IN ('pypi', 'github', 'spec')),
    ...
    -- remaps across restart. The default empty object means every item mounts at
    -- its declared base.
...
-- ============================================================
-- Runs index — one enumerable row per run
-- ============================================================
