...
:class:`~fastmcp.exceptions.ToolError` backed by :class:`PermissionDeniedError`; a
...
class AuthzMiddleware(Middleware):
    """Tool-edge authorization for projected operations.

    Installed on the main ``_fast_mcp`` and on every sub-MCP FastMCP via
    ``add_middleware``.
    """

    def __init__(self, app: Any) -> None:
        """Store the app whose registries and preset manager resolve each dispatch."""
        self._app = app
    ...
    async def on_call_tool(
        ...
        """Authorize a projected-operation dispatch against the caller, then continue the chain.

        A denial raises :class:`~fastmcp.exceptions.ToolError`; a non-operation tool
        passes through.
        """
