"""Manifest/env resolution and the secret seal.

Materialize a PRESERVED document's ``!ENV`` markers for validation and the seal, read the persisted
manifest/env, and derive the effective env a change or profile replace would produce.
"""
...
class _ResolutionMixin(_ConfigServiceBase):
    """The one resolution the pipeline uses for both validation and the secret seal.

    Plus the persisted-store reads and the effective-env derivations.
    """

    def _resolve(self, document: Mapping[str, Any]) -> dict[str, Any]:
        """The RESOLVED projection of a PRESERVED document — ``!ENV`` markers materialized against the env.
    ...
    def _read_preserved_manifest(self) -> dict[str, Any]:
        ...
        A deployment with no manifest registers no backend, so the env-change invariant has nothing to reject.
        """
        try:
    ...
    def _effective_replace_env(self, profile_env: dict[str, str]) -> dict[str, str]:
        """The effective env a profile REPLACE produces, as the reloaded process would see it.
...
def _environ(env: dict[str, str]) -> Iterator[None]:
    """Temporarily replace ``os.environ`` with ``env`` for the duration of the block, restoring it afterwards.

    Used to resolve a manifest's ``!ENV`` markers against a proposed post-change env without mutating the
    real process env.
    """
