...
async def _create_memory_store(store_kwargs: dict[str, Any]) -> tuple[Resource, CleanupFn]:
...
async def _create_sqlite_store(conn_string: str | None, store_kwargs: dict[str, Any]) -> tuple[Resource, CleanupFn]:
...
async def _create_redis_store(conn_string: str | None, store_kwargs: dict[str, Any]) -> tuple[Resource, CleanupFn]:
    if conn_string is None:
...
async def create_store_resource(provider: str, conn_string: str | None = None, **kwargs) -> tuple[Resource, CleanupFn]:
    ...
    match provider:
        case "memory":
            return await _create_memory_store(store_kwargs)
        case "sqlite":
            return await _create_sqlite_store(conn_string, store_kwargs)
        case "postgres":
            return await _create_postgres_store(conn_string, store_kwargs)
        case "redis":
            return await _create_redis_store(conn_string, store_kwargs)
        case _:
