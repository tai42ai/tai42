"""The web invite: a pair code carried into the web chat by the ``?tai_pair=`` URL.
...
async def _await_web_out(web: WebChatClient, submit: Callable[[], Awaitable[object]], needle: str) -> None:
    ...
    ``needle``, exactly as ``test_web_public_chat`` does — the answer is caught whichever side of the stream open
    it lands on."""
