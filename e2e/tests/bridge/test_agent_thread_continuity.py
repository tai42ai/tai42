"""Agent-thread continuity across a linked person's two channels.
...
checkpoint-continuity read ``test_web_public_chat`` uses. Histories are never migrated, so linked memory starts at
