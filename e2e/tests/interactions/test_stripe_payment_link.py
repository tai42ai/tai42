...
Asserts the tool mints exactly one Checkout Session at the stub, stamps the money metadata plus
the caller metadata verbatim, and returns that session's hosted link + id. Deliberately NOT a real
Stripe round trip.
"""
...
async def test_create_stripe_payment_link_mints_a_session(
    ...
    # The tool stamps the money metadata plus any caller metadata; the returned link is the
    # stub's hosted URL.
    assert session["metadata"]["tai_amount"] == str(_AMOUNT), session
    assert session["metadata"]["tai_currency"] == _CURRENCY, session
    assert session["metadata"]["external_ref"] == ref, session
