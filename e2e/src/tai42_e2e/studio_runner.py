...
# discloses that id to the client. Explicit ``TAI_E2E_STACK_HANDOFF`` names the file
# (the playwright config points it at the specs' own directory); otherwise it lands in
# the runner's working directory. Never derived from the package location, which only
# a source checkout lays out next to the specs. Rewritten every boot, removed on
# teardown so a stale file can never point a later run at a released DB.
_STACK_HANDOFF = Path(os.environ.get("TAI_E2E_STACK_HANDOFF") or Path.cwd() / ".stack-handoff.json")
