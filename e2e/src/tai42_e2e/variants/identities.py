...
class IdentityVariant(abc.ABC):
    ...
    @abc.abstractmethod
    def seed_identity(
        self, infra: Infra, resources: StackResources, *, user_id: str, hashed: str, owner_user_id: str
    ) -> None:
        """Write the provider's private identity record for a key whose raw token
        hashes to ``hashed``, carrying the ``owner_user_id`` claim every api key belongs
        to (the harness owns the token + the generic PG policy row; the provider owns
        this storage format)."""
...
class RedisIdentity(IdentityVariant):
    ...
    def seed_identity(
        ...
        # ``ac:key:<sha256(raw)>`` carrying the owner claim, plus the
        # ``ac:management:key:<user>`` reverse lookup on the stack's logical DB.
        client = redis.Redis.from_url(resources.redis_url, decode_responses=True)
        try:
            client.hset(
                f"ac:key:{hashed}",
                mapping={"user_id": user_id, "description": "owner key", "owner_user_id": owner_user_id},
            )
...
class FixtureIdentity(IdentityVariant):
    ...
    def seed_identity(
        ...
        # ``ac:key:*`` hash), carrying the owner claim in its ``owner_user_id`` column.
        # The seed runs before boot, so it ensures the table exists (idempotent) before
        # writing — the provider's own healthcheck ensures it too.
        with psycopg.connect(
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "INSERT INTO fixture_identity_keys (key_hash, user_id, description, owner_user_id) "
                    "VALUES (%s, %s, %s, %s) "
                    ...
                    (hashed, user_id, "owner key", owner_user_id),
                )
            conn.commit()
