...
the fixture reproduces the twilio/whatsapp channel shape where the routes live in a
sibling of the manifest leaf. The skeleton installer's manifest patch persists this
module (before the SPA catch-all) and the epoch build mounts the sibling's routes
into the served ASGI app; a plain in-process config reload must re-fire them.
"""
