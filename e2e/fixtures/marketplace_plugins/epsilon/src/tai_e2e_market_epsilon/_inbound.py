...
side-effect of a SIBLING (the twilio/whatsapp channel shape), not from the leaf
itself. This is what makes an in-process config reload exercise the sibling
re-fire: re-importing only the leaf leaves this module cached, so the routes must
be re-fired by widening the reload to the whole distribution.
