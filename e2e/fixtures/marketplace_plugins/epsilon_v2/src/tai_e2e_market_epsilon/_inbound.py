"""The epsilon fixture's route registrations — the route-carrying SIBLING (bumped).

Same sibling shape as the prior version (the manifest leaf
``tai_e2e_market_epsilon.router`` imports THIS module for its
``@tai42_app.http.custom_route`` side-effect), carrying the extra public
``GET /probe`` route the bumped version adds.

Four routes relative to the item's declared ``e2e-epsilon`` mount base: an authed
``GET /ping``, a public ``GET /open``, an authed ``POST /open`` sharing that path
(the per-method pin), and a public ``GET /probe``. The auth posture of each route
comes from the ``tai-plugin.yml`` ``public`` flag, not a ``custom_route`` argument.
"""
